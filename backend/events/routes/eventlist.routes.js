...
router.get("/", async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from("events")
      .select(
        `event_id, event_title, start_time, end_time, location, event_categories(category_id, category:categories(category_id, category_name))`
      )
...
// Healthcheck: GET /api/events/health
// Performs a lightweight DB query to verify connectivity
